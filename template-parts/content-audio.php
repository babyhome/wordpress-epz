...
<article id="<?php the_id(); ?>" <?php post_class('post'); ?>>
	...
	$audio_embed = rwmb_meta( 'epz_audio_embed_code', $args = array( 'type' => 'oembed' ), $post->ID );
	//$audio_self_hosted 	= rwmb_meta( 'epz_shaudio', $args = array( 'type' => 'file_advanced' ), $post->ID );
	$bg_image 	= 'style="background-image: url(' . esc_url( wp_get_attachment_url( get_post_thumbnail_id( $post->ID ) ) ) . ');"';
	
	?>
		<div class="featured-media">
			<?php echo rwmb_meta( 'epz_audio_embed_code', $args = array('type' => 'oembed'), $post->ID ); ?>
		</div>
